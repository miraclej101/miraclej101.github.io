import {Exp} from './Exp';

export const EXPS_ENG : Exp[] = [
    {
        id: 5,
        job_title:'Bitcoin Data Processing (Big data) Project',
        company:'Université Claude Bernard Lyon 1, computer science department, Villeurbanne France',
        description:`Create a graphical interface to answer typical questions related to Bitcoin transaction data presented in the blockchain. Compare several approaches, 
        code optimized by python and libraries from Pyspark, pandas and Numpy. 
        Visualize the results by the matplotlip library.`,
        duration:'31 Jan 2022– 3 May 2022',
    },
    {
        id: 4,
        job_title:'Internship in web and network development',
        company:"École normale supérieure, Laboratoire de l'Informatique du Parallélisme (LIP) Lyon 7e France",
        description:`Development of a web tool with an ergonomic graphical interface 
        allowing the visualization, parameterization, and execution of scenarios 
        from the Internet of Things (IoT) on a network simulator.`,
        duration:'1 Jul 2021– 31 Aug 2021',
    },
    {
        id: 3,
        job_title:'Investor',
        company:'Self-Employed',
        description:` • Manage my own portfolio of investment in the Stock Exchange 
        of Thailand (SET). 
        • Allocation of owned funds to several mutual funds which invest 
        in Thailand and overseas and invest in real estate in Bangkok, 
        Thailand.`,
        duration:'2004-Present'
    },
    {
        id: 2,
        job_title:'Director ',
        company:'Pinkarjai Furniture, Bangkok, Thailand',
        description:`• Led the process of international sales, and negotiations with 
        foreign customers achieving sales of 1.2 m THB minimum per year 
        • Controlled budget of 1 m THB a month and introduce new 
        facility to enable internet banking minimising the exposure of 
        the company to non-payment.`,
        duration:'2003-2015',
    },
    {
        id: 1,
        job_title:'Production Engineer',
        company:'Thai Polyethylene, Siam Chemical Group (SCG), Thailand',
        description:`• Production control of 2 Polypropylene plants accounting for 
        400,000 tonnes/year
        • Liaised and co-ordinated appropriate stakeholders in the 
        process of product redesign and new product development`,
        duration:'2001-2002',
    },
]

export const EXPS_FR : Exp[]= [
    {
        id: 5,
        job_title:'Traitement des données Bitcoin (Big data)',
        company:'Université Claude Bernard Lyon 1, département informatique, Villeurbanne, France',
        description:`Créer une interface graphique permettant de répondre à des questions typiques
        liées à des données de transactions Bitcoin présentées dans la blockchain. Comparer 
        plusieurs approches, un code optimisé par python et des librairies de Pyspark, 
        pandas et Numpy. Visualiser les résultats par la librairie de matplotlip.`,
        duration:'31 janvier 2022 – 3 mai 2022',
    },
    {
        id: 4,
        job_title:'Stagiaire en informatique (développement web & réseaux)',
        company:"École normale supérieure, Laboratoire de l'Informatique du Parallélisme (LIP) Lyon 7e, France",
        description:`Développement d'un outil web avec une interface graphique ergonomique
        permettant la visualisation, la paramétrisation et l'exécution de scénarios issus 
        de l'Internet des Objets (IoT) sur un simulateur réseau.`,
        duration:'1 juillet 2021 - 31 août 2021',
    },
    {
        id: 3,
        job_title:'Investisseur',
        company:'Auto-entreprise',
        description:`Investissement personnel en bourse de Thaïlande et dans des fonds communs 
        de placement pour l’investissement mondial ainsi que l’immobilier à Bangkok`,
        duration:'Depuis 2004',
    },
    {
        id: 2,
        job_title:'Directrice',
        company:'Pinkrajai Furniture Bangkok, Thaïlande',
        description:`• Gestion des comptes clients / fournisseurs (factures et chiffre d’affaires)
        • Négociation des ventes internationales et accompagnement des sales process.`, 
        duration:'2003-2015',
    },
    {
        id: 1,
        job_title:'Ingénieure de production',
        company:'Thaï Polyéthylène (TPE), Siam Chimique Groupe (SCG) Rayong, Thaïlande',
        description:`• Contrôle de la production annuelle de 400 mille tonnes de polypropylène
        • Participation à l’installation de nouveaux équipements, et de transmission
        des savoirs faire techniques aux techniciens opérationnels (retour d’expérience)`,
        duration:'2001-2002',
    },
]